...
class Customize_Tab extends Tab {
	...
	*/
	public function get_sections() {
		// Nested sections for Videos tab
		$videos_nested_sections = array(
		...
		$sections = array(
			...
			'customize_videos'          => array(
				...
				'nested_sections' => $videos_nested_sections
			),
			...
			array(
				...
				'options'   => array(
					...
					array(
						'value' => 'carousel',
						'icon'  => 'carousel',
						'checkExtension' => sby_is_pro() ? false : 'feedLayout',
	...
	private function get_customize_header_controls() {
		return array(
			...
			array(
				...
				'condition' => ['showheader' => [true], 'headerstyle' => ['standard'], 'type' => ['channel', 'favorites']],
				'conditionHide'	=> true,
				'checkExtensionPopup' => sby_is_pro() ? false : 'subscribersCount',
				'options' => array(
					'enabled'  => true,
					'disabled' => false,
				),
		...
		return array(
			...
			array(
				...
				'style'         => sby_is_pro() ? array(
					...
					'.sb_youtube[data-videostyle=boxed] .sby_items_wrap .sby_item .sby_inner_item' => 'border-radius:{{value}}px!important;',
					'.sb_youtube[data-videostyle=boxed] .sby_items_wrap .sby_video_thumbnail' => sprintf('border-radius:%spx!important;', '{{value}}' )
				),
				'stacked'       => 'true',
			),
			array(
	...
	*/
	private function edit_individual_elements_nested_controls() {
		...
		if ( !sby_is_pro() ) {
			$learn_more = 'https://smashballoon.com/pricing/youtube-feed?utm_campaign=youtube-free&utm_source=individual-elements&utm_medium=learn-more';
			$controls[] = [
				'type'          => 'heading',
				'heading'       => __( 'Advanced', 'feeds-for-youtube' ) . '<span class="sb-breadcrumb-pro-label">PRO</span>',
				'description'   => sprintf(__( 'These properties are available in the PRO version. <a href="%s" target="_blank">Learn More</a>', 'feeds-for-youtube' ), $learn_more),
